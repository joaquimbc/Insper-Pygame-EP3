...
def digita_nome(num_jogador):
    ...
    COR_DO_INPUT = (255, 0, 0) 

    # Cria uma superfice
    tela = pygame.display.set_mode((LARGURA, ALTURA))
    pygame.display.set_caption('Insira o nome do jogador')

    # Cria uma instância de TextInputManager(). Será quem irá armazenar o nome final
    text_input_manager = TextInputManager()

    # Cria uma instância de TextInputVisualizer(). Será quem irá mostrar o texto enquanto é digitado
    visualizador = TextInputVisualizer(manager=text_input_manager,
                                                font_object=pygame.font.Font(pygame.font.get_default_font(), 25),
                                                font_color=COR_DO_TEXTO)

    # Cria um objeto para o texto do nosso input
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    return text_input_manager.value # Retorna a string inserida pelo usuário, armazenada em TextInputManager

        visualizador.update(events)

        # Desenha o fundo
        tela.fill(COR_DE_FUNDO)

        # Desenha o texto pedindo para o usuário inserir o nome
        texto_mensagem = font.render(f"Insira o nome do jogador {num_jogador}:", True, COR_DO_INPUT)
        prompt_text_rect = texto_mensagem.get_rect(center=(LARGURA // 2, ALTURA // 4)) 
        tela.blit(texto_mensagem, prompt_text_rect)

        # Desenha a caixa de input do nome
        superfice_texto = visualizador.surface
        text_rect = superfice_texto.get_rect(center=(LARGURA // 2, ALTURA // 2))
        tela.blit(superfice_texto, text_rect)
